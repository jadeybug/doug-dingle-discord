...
const excludedSections = ["See also", "References", "External links"];
/**
...
const addContent = content => {
  if (content.length > 0) {
    ...
    const markovContent = content.filter(section => !excludedSections.includes(section.title)).reduce((reducedContent, currentSection) => {
      const parsedContent = currentSection.content.replaceAll("\n", "").split(/\. |\./).map(sentence => ({
        title: currentSection.title,
        string: sentence + "."
      })).filter(mappedContent => mappedContent.string.length > 3);
      return [...reducedContent, ...parsedContent];
    }, []);
...
const addLinks = links => {
  return new Promise((resolve, reject) => {
     ...
     **/
    getLinks(links).then(chosenLinks => {
      Promise.all(chosenLinks).then(pages => {
        const linkPromises = [];
        pages.forEach(page => {
          linkPromises.push(page.content().then(addContent));
          linkPromises.push(page.images().then(addImages));
        });
        Promise.all(linkPromises).then(() => resolve(true)).catch(e => reject(e));
      }).catch(e => reject(e));
    });
  });
};
...
const getPage = (query, iteration = 0) => {
  return new Promise((resolve, reject) => {
    wikijs(wikiOptions).find(query).then(resolve).catch(() => wikijs(wikiOptions).random(1).then(([randoPage]) => getPage(randoPage))).catch(() => reject('Your premise is flawed.'));
  });
};
...
client.on('message', msg => {
  var _msg$mentions, _msg$mentions$users;

  if ((_msg$mentions = msg.mentions) !== null && _msg$mentions !== void 0 && (_msg$mentions$users = _msg$mentions.users) !== null && _msg$mentions$users !== void 0 && _msg$mentions$users.find(usr => usr.id === client.user.id)) {
    const query = msg.content.substr(msg.content.indexOf("<@" + client.user.id + ">") + client.user.id.length + 3).trim();
    getPage(query).then(page => {
      return new Promise((resolve, reject) => {
        Promise.all([page.links().then(addLinks), page.content().then(addContent), page.images().then(addImages)]).then(() => resolve(true)).catch(e => reject(e));
      });
    }).then(() => {
      try {
